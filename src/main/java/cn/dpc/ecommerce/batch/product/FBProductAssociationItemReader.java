...
@Slf4j
public class FBProductAssociationItemReader extends BaseItemReader<ProductAssociations> {
    public FBProductAssociationItemReader(DataSource dataSource, int pageSize, int fetchSize) {
    ...
    private static final String fromClause = """
            ...
            left join voucher_products_constraint_outlets vpco
                     on p.id = vpco.product_id
            left join outlets o on vpco.applicable_outlet_uuid = o.uuid
            left join properties pr on pr.marsha_code = vpco.marsha_code
            left join property_promotions pp
                     on pp.product_uuid = p.uuid and pp.deleted_at is null and pp.marsha_code = vpco.marsha_code
            ...
            or exists(select 1
                    ...
                    where pp2.product_uuid = p.uuid and
                          pp2.marsha_code = vpco.marsha_code and pp2.updated_at > :startTime and
            ...
            or vpco.updated_at > :startTime and vpco.updated_at < :endTime
            or vpco.deleted_at > :startTime and vpco.deleted_at < :endTime
            or o.updated_at > :startTime and o.updated_at < :endTime
